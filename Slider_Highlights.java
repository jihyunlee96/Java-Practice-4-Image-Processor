...
public class Slider_Highlights extends JSlider implements ChangeListener {
	...
	public Slider_Highlights (int x, int y)
	{
	...
	{	
		...
		Main.frame.image.draw_image(change_highlights(factor), false);		
	}
	
	class MyMouseListener extends MouseAdapter implements MouseMotionListener
	{			
		public void mouseReleased (MouseEvent e) {			
			Main.frame.image.change_currentImg(change_highlights(factor));
		}
	}
	
	// returns an instance of BufferedImage with changed brightness
	public Integer[][][] change_highlights (int factor)
	{							
		...
		for (int i = 0; i < width; i ++) {
			for (int j = 0; j < height; j ++) {
				...
				// if the pixel is 'bright', add factor
				if (r + g + b > 600) {
					r += factor;
					g += factor;
					b += factor;
